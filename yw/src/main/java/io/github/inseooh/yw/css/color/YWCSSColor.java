...
@YWCSSType
public abstract class YWCSSColor {
    public abstract Color toAWTColor();
    ...
    public static final YWCSSColor CANVAS_TEXT = new YWCSSRGBColor(0, 0, 0, 255);
    ...
    private static int parseNumberOrPercentageComponent(YWCSSTokenStream ts) throws YWSyntaxError {
        ...
        try {
            YWCSSLength v = YWCSSLength.Percentage.parsePercentage(ts);
    ...
    private static YWCSSColor parseLegacyRGBSyntax(YWCSSToken.ASTFunction func) throws YWSyntaxError {
        ...
        YWCSSLength[] per = ts.parseCommaSeparatedRepeation(new YWCSSLength[0], () -> {
            return YWCSSLength.Percentage.parsePercentage(ts);
        });
        if (per.length == 3) {
            // Percentage value
